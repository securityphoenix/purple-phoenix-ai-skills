...
| `401 Unauthorized` (REST), empty body | **Wrong credential type** — a `phx_live_` API key is being sent where a `phx_at_` access token is required. The protected `/api/v1/external/mcp/**` endpoints reject any other prefix before looking it up, so there is no body and no server log line. The mint route `/api/v1/external/auth/token` is the exception and takes the API key | Exchange it: `printf 'Authorization: Bearer %s\n' "$PHX_MCP_TOKEN" \| curl -sS -X POST -H @- -H 'Content-Type: application/json' -d '{}' "$BASE/api/v1/external/auth/token" \| jq -r .accessToken` |
